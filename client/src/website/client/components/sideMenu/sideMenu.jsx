...
const SideMenu = ({ sideMenu_show }) => {
    return (
        <div onClick={(e) => e.stopPropagation()} className={`${sideMenu_show.sideMenu_state === 'close' ? 'block' : 'hidden'} select-none md:block bg-slate-800 absolute md:fixed bottom-0 left-0 px-2 py-4 top-12 w-full sm:w-52 md:w-[25%] lg:w-[20%] text-white`}>
            ...
            <Link to="/member/referrals-income" className='space-x-1 block px-3 py-2 rounded-lg hover:bg-slate-950'>
                <span className="align-middle">
                    <ion-icon name="person-add-outline"></ion-icon>
                </span>
                <span>Referrals Income</span>
            </Link>
